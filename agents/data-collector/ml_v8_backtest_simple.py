...
FACTORS = [
    ...
    'main_flow_5d', 'main_flow_20d'
]
# ===============================

print("=" * 60)
print("🤖 V8 简化版回测 - 资金流向纯享版")
print("=" * 60)
print(f"时间：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
print("=" * 60)
print(f"\n📊 策略调整：")
print(f"  因子数量：12 个 → 4 个（只用资金流向）✅")
print(f"  持有周期：5 天 → {HOLDING_PERIOD} 天（长线）✅")
...
# 加载 V8 模型（需要重新训练只使用 4 个因子的模型）
# 但为了快速测试，我们先用简单规则代替模型预测
print(f"\n⚠️  使用简化规则：按 north_flow_5d 排序选股")
print("=" * 60)
...
for date in dates[:-HOLDING_PERIOD]:
    ...
    # 买入新持仓（按 north_flow_5d 排序）
    if len(positions) < TOP_K:
        available = TOP_K - len(positions)
        # 按 north_flow_5d 降序排序，选前 TOP_K
        day_data_sorted = day_data.sort_values('north_flow_5d', ascending=False)
...
output_file = output_dir / f"backtest_result_v8_simple_{timestamp}.json"
