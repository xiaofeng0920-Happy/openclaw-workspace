...
def get_a_stock_financials(symbol):
    """获取 A 股财务数据"""
    try:
        import akshare as ak
        ...
        if df is not None and len(df) > 0:
            ...
            return {
                ...
                'pb_ratio': float(latest.get('市净率', 2)),
                'roe': float(latest.get('净资产收益率', 10)),
                'roa': float(latest.get('总资产收益率', 5)),
                'debt_to_equity': float(latest.get('资产负债率', 50)) / 100,
                'profit_margin': float(latest.get('销售净利率', 15)),
                'dividend_yield': float(latest.get('股息率', 2)),
            }
    except:
        pass
...
def prepare_dataset():
    ...
    for i, symbol in enumerate(A50_STOCKS, 1):  # 使用全部 95 只
        df = get_a_stock_history(symbol)
        if len(df) > 0:
            df = calculate_factors(df)
            
            # 财务数据
            financials = get_a_stock_financials(symbol)
            if financials:
...
def train_model(dataset):
    ...
    print(f"\n📊 评估结果:")
    print(f"  IC:   {metrics['ic']:.4f} (V3: 0.0408, 目标：>0.05)")
...
def main():
    ...
    result['model'].booster_.save_model(str(output_dir / f"lgbm_v4_a50_{ts}.json"))
    with open(output_dir / f"metrics_v4_a50_{ts}.json", 'w') as f:
        json.dump({
            'version': 'v4',
            'data_source': 'a50_stocks',
