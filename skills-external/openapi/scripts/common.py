...
_AUTHORITY_URLS = {
    "futu": "https://openapi.futunn.com/futu-api-doc/intro/authority.html",
    "moomoo": "https://openapi.moomoo.com/moomoo-api-doc/en/intro/authority.html",
}
...
def _get_authority_url():
    """根据 FUTU_BRAND 环境变量返回对应行情权限页面链接"""
    brand = os.getenv("FUTU_BRAND", "").strip().lower()
    return _AUTHORITY_URLS.get(brand)


def _build_permission_hint():
    """构建行情权限不足的提示信息（市场名称和链接自适应）"""
    ...
    return (
        f"\n\n{market_prefix}行情权限不足，请购买对应行情卡以获取数据。详情参考：\n"
        f"  牛牛用户: {_AUTHORITY_URLS['futu']}\n"
        f"  moomoo 用户: {_AUTHORITY_URLS['moomoo']}"
    )
...
def _build_permission_hint_json():
    ...
    return {
        "hint": hint,
        "futu_authority_url": _AUTHORITY_URLS["futu"],
        "moomoo_authority_url": _AUTHORITY_URLS["moomoo"],
    }
