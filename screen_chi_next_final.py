...
def get_latest_trade_date():
    """获取最新交易日"""
    try:
        df = pro.trade_cal(exchange='SSE', start_date='20260301', end_date='20260430', is_open='1')
        if not df.empty:
...
def main():
    print("=" * 80)
    print("A 股创业板 (300xxx) 股票筛选 - 最终优化版")
    ...
    # 获取 PE 数据
    pe_dict = get_pe_for_all_stocks(ts_codes, trade_date)
    print(f"获取到 {len([v for v in pe_dict.values() if v is not None and v > 0])} 只股票的有效 PE 数据")
